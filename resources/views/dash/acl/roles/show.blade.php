    ...
    <div class="col-xl-8">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <h4>{{ $role->label }}</h4>
                <hr>
                <form action="{{ route('roles.delete', $role->id) }}" method="POST" onsubmit="return confirm('Deseja realmente remover esse registro?');">
                    @csrf
					@method('DELETE')
                    <button type="submit" class="btn btn-danger">Remover função</button>
                </form>
            </div>
        </div>
